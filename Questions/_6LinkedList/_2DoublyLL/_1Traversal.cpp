...
// Time Complexity: O(n)
// The time complexity is O(n) because we are iterating through the vector of size n exactly once to construct the linked list.

// Space Complexity: O(n)
// The space complexity is O(n) because we are creating n nodes, one for each element in the vector.
